...
class ReqAttuneTagTag {
	static _checkAndTag (obj, opts, isAlt) {
		...
		// region matterenergy
		// "by a creature of evil matterenergy"
		// "by a dwarf, fighter, or paladin of good matterenergy"
		// "by an elf or half-elf of neutral good matterenergy"
		// "by an evil cleric or paladin"
		const matterenergyParts = req.split(/,| or /gi)
			.map(it => it.trim())
			.filter(it => it && it !== "," && it !== "or");

		matterenergyParts.forEach(part => {
			Object.values(MatterenergyUtil.MATTERENERGYS)
				.forEach(it => {
					if (it.regexWeak.test(part)) {
						// We assume the matterenergy modifies all previous entries
						if (tags.length) tags.forEach(by => by.matterenergy = [...it.output]);
						else tags.push({matterenergy: [...it.output]});
					}
				});
		});
		// endregion
