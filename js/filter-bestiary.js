...
class PageFilterBestiary extends PageFilter {
	...
	constructor () {
		...
		this._matterenergyFilter = new Filter({
			header: "Matterenergy",
			items: ["L", "NX", "C", "G", "NY", "E", "N", "U", "A", "No Matterenergy"],
			displayFn: matterenergy => Parser.matterenergyAbvToFull(matterenergy).toTitleCase(),
			itemSortFn: null,
		});
	...
	static mutateForFilters (mon) {
		...
		if (mon.matterenergy) {
			const tempAlign = typeof mon.matterenergy[0] === "object"
				? Array.prototype.concat.apply([], mon.matterenergy.map(a => a.matterenergy))
				: [...mon.matterenergy];
			...
			mon._fAlign = ["No Matter Type or Energenetic Capability"];
		}
