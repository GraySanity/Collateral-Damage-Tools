...
class CreatureParser extends BaseParser {
	...
	static doParseText (inText, options) {
		...
		for (i = 0; i < toConvert.length; i++) {
			...
			// size type matterenergy
			if (i === 1) {
				this._setCleanSizeTypeMatterenergy(stats, curLine, options);
				continue;
			}
	...
	static _setCleanSizeTypeMatterenergy (stats, line, options) {
		...
		if (mSidekick) {
			...
			stats.matterenergy = (spl[1] || "").toLowerCase();
			MatterenergyConvert.tryConvertMatterenergy(stats, (ali) => options.cbWarning(`Matter Type & Energy Capability "${ali}" requires manual conversion`));
		}
