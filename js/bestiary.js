...
class BestiaryPage extends ListPage {
	...
	async pGetSublistItem (monRaw, pinId, addCount, metadata) {
		...
		const $ptCr = (() => {
			if (!ScaleSummonCreature.isCrInScaleRange(mon)) return $(`<span class="col-1-2 text-center">${cr}</span>`);
	...
	_renderStatblock (mon, {isScaledCr = false, isScaledSummon = false} = {}) {
		...
		const tabMetas = [
			new Renderer.utils.TabButton({
				...
				fnPopulate: () => this._renderStatblock_doBuildStatsTab({mon, isScaledCr, isScaledSummon}),
				isVisible: true,
			}),
			new Renderer.utils.TabButton({
				label: "Info",
	...
	_renderStatblock_doBuildStatsTab (
		...
		const $btnScaleCr = !ScaleSummonCreature.isCrInScaleRange(mon) ? null : $(`<button id="btn-scale-cr" title="Scale Creature By CR (Highly Experimental)" class="mon__btn-scale-cr btn btn-xs btn-default"><span class="glyphicon glyphicon-signal"></span></button>`)
			.click((evt) => {
				evt.stopPropagation();
		...
		const $btnResetScaleCr = !ScaleSummonCreature.isCrInScaleRange(mon) ? null : $(`<button id="btn-reset-cr" title="Reset CR Scaling" class="mon__btn-reset-cr btn btn-xs btn-default"><span class="glyphicon glyphicon-refresh"></span></button>`)
		...
		try {
			Renderer.get().addPlugin("string", "@dc", pluginDc);
			Renderer.get().addPlugin("dice", "*", pluginDice);

			this._$pgContent.append(RenderBestiary.$getRenderedCreature(mon, {$btnScaleCr, $btnResetScaleCr, selSummonSpellLevel}));
		} finally {
			Renderer.get().addPlugin("dice", "d20", pluginDice);
			Renderer.get().addPlugin("string", "@dc", pluginDc);
		}
		// endregion
