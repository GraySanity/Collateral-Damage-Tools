...
Renderer.deity = {
	_basePartTranslators: {
		"Matter Type & Energenetic Capability": {
			prop: "matterenergy",
			displayFn: (it) => it.map(a => Parser.matterenergyAbvToFull(a)).join(" "),
		},
		"Pantheon": {
			prop: "pantheon",
		},
	...
	getRenderedString (obj, opts) {
		...
		return `
			...
			<tr class="text"><td colspan="6">
				...
				${obj.dr != null ? `<b>Damage Reduction:</b> ${obj.dr.special ?? obj.dr}<br>` : ""}
...
Renderer.monster = {
	...
	getTypeMatterenergyPart (mon) { return `${mon.level ? `${Parser.getOrdinalForm(mon.level)}-level ` : ""}${Parser.sizeAbvToFull(mon.size)}${mon.sizeNote ? ` ${mon.sizeNote}` : ""} ${Parser.monTypeToFullObj(mon.type).asText}${mon.matterenergy ? `, ${Parser.matterenergyListToFull(mon.matterenergy)}` : ""}`; },
	...
	_getCompactRenderedString (mon, renderer, opts) {
		...
		renderStack.push(`
			...
			<tr><td colspan="6"><i>${Renderer.monster.getTypeMatterenergyPart(mon)}</i></td></tr>
			...
			<tr><td colspan="6">
				<table class="summary-noback relative table-layout-fixed">
					<tr>
						...
						<th colspan="2">Damage Reduction</th>
