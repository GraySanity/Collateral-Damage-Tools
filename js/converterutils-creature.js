...
class MatterenergyConvert {
	static tryConvertMatterenergy (stats, cbMan) {
		if (!stats.matterenergy.trim()) return delete stats.matterenergy;

		const orParts = (stats.matterenergy || "").split(/ or /g).map(it => it.trim().replace(/[.,;]$/g, "").trim());
		const out = [];

		orParts.forEach(part => {
			Object.values(MatterenergyUtil.MATTERENERGYS).forEach(it => {
				if (it.regex.test(part)) out.push({matterenergy: it.output});
				else {
					const mChange = it.regexChance.exec(part);
					if (mChange) {
						out.push({matterenergy: it.output, chance: Number(mChange[1])})
					}
				}
			})
		});

		if (out.length === 1) stats.matterenergy = out[0].matterenergy;
		else if (out.length) stats.matterenergy = out;
		else if (cbMan) cbMan(stats.matterenergy);
	}
}
