...
class CreatureBuilder extends Builder {
	...
	async pHandleSidebarLoadExistingData (creature, opts) {
		...
		if (ScaleSummonCreature.isCrInScaleRange(creature) && !opts.isForce) {
	...
	_renderInputMain () {
		...
		this.__$getMatterenergyInput(cb).appendTo(infoTab.$wrpTab);
		...
		this.__$getDRInput(cb).appendTo(defenseTab.$wrpTab);
	...
	__$getMatterenergyInput (cb) {
		const [$row, $rowInner] = BuilderUi.getLabelledRowTuple("Matterenergy", {isMarked: true});

		const doUpdateState = () => {
			const raw = matterenergyRows.map(row => row.getMatterenergy());
			if (raw.some(it => it && (it.special != null || it.matterenergy !== undefined)) || raw.length > 1) {
				this._state.matterenergy = raw.map(it => {
					if (it && (it.special != null || it.matterenergy)) return it;
					else return {matterenergy: it};
				})
			} else this._state.matterenergy = raw[0];
			cb();
		};

		const matterenergyRows = [];

		const $wrpRows = $(`<div/>`).appendTo($rowInner);

		if ((this._state.matterenergy && this._state.matterenergy.some(it => it && (it.special != null || it.matterenergy !== undefined))) || !~CreatureBuilder.__$getMatterenergyInput__getMatterenergyIx(this._state.matterenergy)) {
			this._state.matterenergy.forEach(matterenergy => CreatureBuilder.__$getMatterenergyInput__getMatterenergyRow(doUpdateState, matterenergyRows, matterenergy).$wrp.appendTo($wrpRows));
		} else {
			CreatureBuilder.__$getMatterenergyInput__getMatterenergyRow(doUpdateState, matterenergyRows, this._state.matterenergy).$wrp.appendTo($wrpRows)
		}

		const $wrpBtnAdd = $(`<div/>`).appendTo($rowInner);
		$(`<button class="btn btn-xs btn-default">Add Matter Type and Energenetic Capability</button>`)
			.appendTo($wrpBtnAdd)
			.click(() => {
				CreatureBuilder.__$getMatterenergyInput__getMatterenergyRow(doUpdateState, matterenergyRows).$wrp.appendTo($wrpRows);
				doUpdateState();
			});
	...
	static __$getMatterenergyInput__getMatterenergyRow (doUpdateState, matterenergyRows, matterenergy) {
		const initialMode = matterenergy && matterenergy.chance ? "1" : matterenergy && matterenergy.special ? "2" : (matterenergy === null || (matterenergy && matterenergy.matterenergy === null)) ? "3" : "0";

		const getMatterenergy = () => {
			switch ($selMode.val()) {
				case "0": {
					return [...CreatureBuilder._MATTERENERGYS[$selAlign.val()]];
				}
				case "1": {
					const out = {matterenergy: [...CreatureBuilder._MATTERENERGYS[$selAlign.val()]]};
		...
		const $selMode = $(`<select class="form-control input-xs mb-2">
				<option value="0">Basic Matterenergy</option>
				<option value="1">Chance-Based Matterenergy/Matterenergy with Note</option>
				<option value="2">Special Matterenergy</option>
				<option value="3">No Matterenergy (Sidekick)</option>
		...
		const $selAlign = $(`<select class="form-control input-xs mb-2">${CreatureBuilder._MATTERENERGYS.map((it, i) => it ? `<option value="${i}">${Parser.matterenergyListToFull(it).toTitleCase()}</option>` : `<option disabled>\u2014</option>`).join("")}</select>`)
			.change(() => doUpdateState());
		const $stageSingle = $$`<div>${$selAlign}</div>`.toggleVe(initialMode === "0" || initialMode === "1");
		initialMode === "0" && matterenergy && $selAlign.val(CreatureBuilder.__$getMatterenergyInput__getMatterenergyIx(matterenergy.matterenergy || matterenergy));
		initialMode === "1" && matterenergy && $selAlign.val(CreatureBuilder.__$getMatterenergyInput__getMatterenergyIx(matterenergy.matterenergy));

		// MULTIPLE CONTROLS
		const $iptChance = $(`<input class="form-control form-control--minimal input-xs mr-2" min="1" max="100" placeholder="Chance of matterenergy">`)
			.change(() => doUpdateState());
		const $iptNote = $(`<input class="form-control form-control--minimal input-xs mx-1" placeholder="Matterenergy note">`)
			.change(() => doUpdateState());
		...
		if (initialMode === "1" && matterenergy) {
			$iptChance.val(matterenergy.chance);
			$iptNote.val(matterenergy.note);
		}
		...
		initialMode === "2" && matterenergy && $iptSpecial.val(matterenergy.special);

		const $btnRemove = $(`<button class="btn btn-xs btn-danger mkbru__btn-rm-row mb-2" title="Remove Matterenergy"><span class="glyphicon glyphicon-trash"/></button>`)
			.click(() => {
				matterenergyRows.splice(matterenergyRows.indexOf(out), 1);
				$wrp.empty().remove();
				doUpdateState();
			});
		...
		const out = {$wrp, getMatterenergy};
		matterenergyRows.push(out);
		return out;
	}

	static __$getMatterenergyInput__getMatterenergyIx (matterenergy) {
		return CreatureBuilder._MATTERENERGYS.findIndex(it => CollectionUtil.setEq(new Set(it), new Set(matterenergy)))
	}
	...
	__$getDRInput (cb) {
		const [$row, $rowInner] = BuilderUi.getLabelledRowTuple("Damage Reduction", {isMarked: true});

		const doUpdateState = () => {
			this._state.dr = drRows.map($row => $row.getDr());
			cb();
		};
		...
		const $wrpRows = $(`<div/>`).appendTo($rowInner);
		this._state.dr.forEach(dr => CreatureBuilder.__$getDRInput__getDrRow(dr, drRows, doUpdateState).$wrp.appendTo($wrpRows));

		const $wrpBtnAdd = $(`<div/>`).appendTo($rowInner);
		$(`<button class="btn btn-xs btn-default">Add Damage Reduction Source</button>`)
			.appendTo($wrpBtnAdd)
			.click(() => {
				CreatureBuilder.__$getDRInput__getDrRow(null, drRows, doUpdateState).$wrp.appendTo($wrpRows);
				doUpdateState();
			});
	...
	static __$getDRInput__getDrRow (dr, drRows, doUpdateState) {
		const initialMode = dr && dr.special ? "2" : dr && dr.from ? "1" : "0";

		const getDr = () => {
		//	const drValRaw = UiUtil.strToInt($iptDr.val(), 0, {fallbackOnNaN: 0});
			const drValRaw = $iptDr.val();
			const drVal = isNaN(drValRaw) ? drValRaw : drValRaw;
			const condition = $iptCond.val().trim();
			const braces = $cbBraces.prop("checked");
			...
			switch ($selMode.val()) {
				...
				case "1": {
					const froms = fromRows.map(it => it.getDrFrom()).filter(Boolean);
					if (froms.length) {
						const out = {
							dr: drVal,
							from: froms,
		...
		const $selMode = $(`<select class="form-control input-xs mkbru_mon__dr-split">
				<option value="0">Unarmored</option>
				<option value="1">Damage Reduction From...</option>
				<option value="2">Special</option>
			</select>`).val(initialMode).change(() => {
			switch ($selMode.val()) {
				case "0": {
				...
				case "1": {
					...
					$iptDr.showVe();
					$iptSpecial.hideVe();
					if (!fromRows.length) CreatureBuilder.__$getDRInput__getFromRow(null, fromRows, doUpdateState).$wrpFrom.appendTo($wrpFromRows);
					doUpdateState();
					break;
				}   
				case "2": {
		...
		const $iptDr = $(`<input class="form-control form-control--minimal input-xs mr-2 mkbru_mon__dr-split">`)
			.val(dr && dr.special == null ? dr.dr || dr : 0)
			.change(() => doUpdateState())
			.toggleVe(initialMode !== "2");

		const $iptSpecial = $(`<input class="form-control form-control--minimal input-xs mr-2 mkbru_mon__dr-split">`)
			.val(dr && dr.special ? dr.special : null)
			.change(() => doUpdateState())
			.toggleVe(initialMode === "2");

		const $iptCond = $(`<input class="form-control form-control--minimal input-xs" placeholder="when...">`)
			.change(() => doUpdateState());
		if (dr && dr.condition) $iptCond.val(dr.condition);
		const $cbBraces = $(`<input type="checkbox" class="mkbru__ipt-cb--plain">`)
			.change(() => doUpdateState());
		if (dr && dr.braces) $cbBraces.prop("checked", dr.braces);

		// "FROM" CONTROLS
		const fromRows = [];

		const $wrpFromRows = $(`<div/>`);
		if (dr && dr.from) dr.from.forEach(f => CreatureBuilder.__$getDRInput__getFromRow(f, fromRows, doUpdateState).$wrpFrom.appendTo($wrpFromRows));

		const $btnAddFrom = $(`<button class="btn btn-xs btn-default mb-2">Add Another Feature/Item</button>`)
			.click(() => {
				CreatureBuilder.__$getDRInput__getFromRow(null, fromRows, doUpdateState).$wrpFrom.appendTo($wrpFromRows);
				doUpdateState();
			});
		const $stageFrom = $$`<div class="mb-2 flex-col">
		${$wrpFromRows}
		...
		// REMOVE CONTROLS
		const $btnRemove = $(`<button class="btn btn-xs btn-danger mkbru__btn-rm-row mb-2" title="Remove DR Source"><span class="glyphicon glyphicon-trash"/></button>`)
			.click(() => {
				drRows.splice(drRows.indexOf(out), 1);
				$wrp.empty().remove();
				doUpdateState();
			});

		const $wrp = $$`<div class="flex-col mkbru__wrp-rows mkbru__wrp-rows--removable">
			<div class="flex-v-center mb-2">${$iptDr}${$iptSpecial}${$selMode}</div>
			${$$`<div>${$stageFrom}</div>`}
			<div class="flex-v-center mb-2"><span class="mr-2 mkbru__sub-name--50">Condition</span>${$iptCond}</div>
			<label class="flex-v-center mb-2"><span class="mr-2 mkbru__sub-name--50">Surround with brackets</span>${$cbBraces}</label>
			${$$`<div class="text-right">${$btnRemove}</div>`}
	...
	static __$getDRInput__getFromRow (from, fromRows, doUpdateState) {
		const getDrFrom = () => $iptFrom.val().trim();

		const $iptFrom = $(`<input class="form-control form-control--minimal input-xs mr-2" placeholder="From...">`)
			.change(() => doUpdateState());
		if (from) $iptFrom.val(from);

		const menu = ContextUtil.getMenu(Object.keys(CreatureBuilder._DR_COMMON).map(k => {
			return new ContextUtil.Action(
				k,
				() => {
					$iptFrom.val(CreatureBuilder._DR_COMMON[k]);
					doUpdateState();
				},
			)
		}));
		...
		const $btnRemove = $(`<button class="btn btn-xs btn-danger mkbru__btn-rm-row--nested-1 ml-2" title="Remove DR Feature/Item"><span class="glyphicon glyphicon-trash"/></button>`)
			.click(() => {
				fromRows.splice(fromRows.indexOf(outFrom), 1);
				$wrpFrom.empty().remove();
				ContextUtil.deleteMenu(menu);
		...
		const outFrom = {$wrpFrom, getDrFrom};
		fromRows.push(outFrom);
		return outFrom;
	}
	...
	__$getCrInput (cb) {
		...
		const initialMode = this._state.cr != null
			? this._state.cr.lair ? "1" : this._state.cr.coven ? "2" : ScaleSummonCreature.isCrInScaleRange(this._state) ? "0" : "3"
			: "4";
...
CreatureBuilder._MATTERENERGYS = [
	["MF"],
	["MS"],
	["MH"],
	["MN"],
