...
class PageFilterEquipment extends PageFilter {
	constructor () {
		...
		this._sourceFilter = new Filter({
			header: "Sources",
			deselFn: (it) => PageFilterItems._DEFAULT_HIDDEN_SOURCES.has(it),
			displayFn: Parser.sourceJsonToFull
		});
		...
		this._costFilter = new RangeFilter({header: "Cost", min: 0, max: 1000, isAllowGreater: true, suffix: " credits"});
		this._weightFilter = new RangeFilter({header: "Weight", min: 0, max: 100, isAllowGreater: true, suffix: " kg"});
...
class PageFilterItems extends PageFilterEquipment {
	...
	static _getBaseItemDisplay (baseItem) {
		...
		if (source.toLowerCase() === SRC_CDPH.toLowerCase()) return name;
			...
			Object.entries(tagSet)
				.forEach(([prop, val]) => {
					switch (prop) {
						...
						case "materialenergy": out.push(`Matter Type & Energenetic Capability: ${Parser.materialenergyListToFull(val).toTitleCase()}`); break;
